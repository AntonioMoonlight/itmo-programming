...
public class NetworkAppController {
    ...
    private final Set<String> clientOnlyCommands = Set.of("execute_script");
    ...
    private void handleClientCommand(ParsedCommand parsedCommand) {
        ...
        if ("execute_script".equals(commandName)) {
            handleExecuteScript(parsedCommand.args());
        }
    }
    ...
    private void handleExecuteScript(String[] args) {
        ...
        try {
            ...
            consoleView.println("Error executing script: " + e.getMessage());
        } finally {
    ...
    private void processServerCommand(ParsedCommand parsedCommand) {
        try {
            ...
            if (needsObjectData(parsedCommand.name())) {
